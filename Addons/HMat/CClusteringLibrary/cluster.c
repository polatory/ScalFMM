  ...
  { int loop;
    ...
    { if (j==nl && i==nr)
        ...
        { int k;
          double xmax = x[0];
          double xmin = x[n-1];
          for (k = lo; k <= j; k++) xmax = ccl_max(xmax,x[k]);
          for (k = i; k <= hi; k++) xmin = ccl_min(xmin,x[k]);
...
{ int i, j, k, i1, k1, l1, its;
  ...
  { /* Householder reduction to bidiagonal form */
    ...
    { l = i + 1;
      ...
      anorm = ccl_max(anorm,fabs(w[i])+fabs(rv1[i]));
    }
...
{ int j;
  ...
  { int is = 1;
    ...
    /* Fix the distances */
    for (j = 0; j < js; j++)
      distmatrix[js][j] = ccl_max(distmatrix[is][j],distmatrix[js][j]);
    for (j = js+1; j < is; j++)
      distmatrix[j][js] = ccl_max(distmatrix[is][j],distmatrix[j][js]);
    for (j = is+1; j < n; j++)
      distmatrix[j][js] = ccl_max(distmatrix[j][is],distmatrix[j][js]);
