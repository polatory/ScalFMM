...
class TestLagrangeMpiDirect : public FUTesterMpi<TestLagrangeMpiDirect>{
  ...
  void RunTest(){
    ...
    //Check datas
    {
      ...
      //Summarize
      FMath::FAccurater* FXYZP = new FMath::FAccurater[app.global().processCount()*4];
      FMath::FAccurater* fxyzp = new FMath::FAccurater[4];
      fxyzp[0] = fx;
      fxyzp[1] = fy;
      fxyzp[2] = fz;
      fxyzp[3] = potentialDiff;
      MPI_Gather(fxyzp,4*sizeof(FMath::FAccurater),MPI_BYTE,FXYZP,4*sizeof(FMath::FAccurater),MPI_BYTE,0,app.global().getComm());
