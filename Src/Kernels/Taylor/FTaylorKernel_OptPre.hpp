...
public:
	   ...
	   const int /*size*/, const int inLevel)
  {
	...
	FReal * psiVector = M2LpreComputedDerivatives[inLevel][idxNeigh];
	...
	for(int i=0, orn=0 ; orn<=P ; ++orn){ //i increments along SizeVector values
	  for(al=orn ; al>=0 ; --al)
	    {
	      alpha = orn-al;
	      if (alpha == 1) //Case (al,bl,cl) = (orn-1,1,0) OR (orn-1,0,1)
		{
		  FReal fctl = factorials[al];
