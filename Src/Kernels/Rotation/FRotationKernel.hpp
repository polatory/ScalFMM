...
class FRotationKernel : public FAbstractKernels<ParticleClass,CellClass,ContainerClass> {    
    ...
    FReal d_lmk_analytical(const FReal cosTheta, const FReal sinTheta, const int l , const int m, const int k){
        if( l >= 0 && -l <= k && k <= l && FMath::Abs(k) <= m && m <= l ){
            ...
            for(int n = FMath::Max(-(m+k),0) ; n <= FMath::Min(l-m,l-k) ; ++n){
                sum += FMath::pow(FReal(-1.0), l-m-n) * FReal(combin(l-k, n)) * FReal(combin(l+k, l-m-n)) * FMath::pow(FReal(1.0)+cosTheta,n) * FMath::pow(FReal(1.0)-cosTheta, l-m-n);
            }
            return (FReal(1.0)/FMath::pow(FReal(2.0),l)) * FMath::Sqrt(FReal(fact(l-m)*fact(l+m))/FReal(fact(l-k)*fact(l+k)))
    ...
    /** This function rotate a multipole vector by an angle azimuth phi
      ...
      *
      * The computation is simply a multiplication per a complex number e^{-i \phi m}
        ...
        for(int l = 0 ; l <= P ; ++l){
            for(int m = 0 ; m <= l ; ++m ){
                ...
                for(int k = -l ; k < 0 ; ++k){
                    ...
                    const FReal factor = FMath::Sqrt(FReal(fact(l-abs(m))*fact(l+abs(m)))/FReal(fact(l-k)*fact(l+k)));
                    w_lkm_real += FMath::pow(FReal(-1.0),-k) * factor * d_lmk * vec[atLm(l,-k)].getReal(); // k<0 => Conjugate * -1^k
                    w_lkm_imag -= FMath::pow(FReal(-1.0),-k) * factor * d_lmk * vec[atLm(l,-k)].getImag(); // k<0 => Conjugate * -1^k
                }
                for(int k = 0 ; k <= l ; ++k){
    ...
    /** This function rotate a multipole vector by a angles inclination & azimuth
      ...
      */
    void rotateMultipole(FComplexe vec[], FReal azimuth, FReal inclination){
        rotateMultipoleAroundZ(vec,-azimuth);
    ...
    /** This function rotate a multipole vector by a angles inclination & azimuth
      ...
      * Rotation are not commutative so we have to do it in the right order
      */
    void deRotateMultipole(FComplexe vec[], FReal azimuth, FReal inclination){
        rotateMultipoleAroundY(vec,inclination);
        rotateMultipoleAroundZ(vec,azimuth);
    }
    ...
    void rotateTaylor(FComplexe vec[], const FReal azimuth, const FReal inclination){
        rotateTaylorAroundZ(vec,-inclination);
        rotateTaylorAroundY(vec,-azimuth);
    }
    /** This function rotate a local vector by a angles inclination & azimuth
      * The formula used is present in several paper, but we refer to
      * Implementation of rotation-based operators for Fast Multipole Method in X10
...
public:
            ...
            for(int l = 0 ; l <= P ; ++l ){
                for(int m = 0 ; m <= l ; ++m ){
                    const FReal magnitude = q * (FMath::pow( a , l )/fact(l+m))
                            * legendre[atLm(l,m)];
                    w[atLm(l,m)].setReal(magnitude * FMath::Cos(FReal(m) * sph.getPhi() + i_pow_m[m & 0x3]));
                    w[atLm(l,m)].setImag(magnitude * FMath::Sin(FReal(m) * sph.getPhi() + i_pow_m[m & 0x3]));
                }
            }
            // Goto next particle
    ...
    void M2M(CellClass* const FRestrict inPole, const CellClass*const FRestrict *const FRestrict inChildren, const int inLevel) {
        ...
        for(int idxChild = 0 ; idxChild < 8 ; ++idxChild){
            ...
            if(inChildren[idxChild]){
                ...
                //delete me
                printf("getAzimuth %f getInclination %f r %f\n", sph.getAzimuth(), sph.getInclination(), sph.getR());
                //end delete me
    ...
    void M2L(CellClass* const FRestrict inLocal, const CellClass* inInteractions[], const int /*inSize*/, const int inLevel) {
        ...
        for(int idxNeigh = 0 ; idxNeigh < 343 ; ++idxNeigh){
            ...
            if(inInteractions[idxNeigh]){
                ...
                for(int l = 0 ; l <= P ; ++l ){
                    for(int m = 0 ; m <= l ; ++m ){
                        // u{l,m}(a-b) = sum(j=|m|:P, (j+l)!/b^(j+l+1) w{j,-m}(a)
                        FReal u_lm_real = 0.0;
                        FReal u_lm_imag = 0.0;
                        for(int j = m ; j <= P ; ++j ){
                            const FReal coef = FMath::pow(FReal(-1.0),m) * ( FReal(fact(j+l)) / FMath::pow(b,j+l+1));
    ...
    void L2P(const CellClass* const inLocal, ContainerClass* const inParticles){
        ...
        while( iterParticle.hasNotFinished()){
            ...
            computeLegendre(legendre, sph.getCosTheta(), sph.getSinTheta());
            {
                ...
                for(int l = 1 ; l <= P ; ++l){
                    {
                        const FReal coef = FMath::pow(FReal(-1.0),l) * FMath::pow(r,l) * legendre[atLm(l,0)] / FReal(fact(l+0));
                    ...
                    {
                        const FReal coef = (FMath::pow(FReal(-1.0),l)/FReal(fact(l+0))) * FMath::pow(r,l) * ((FReal(l) * sph.getCosTheta()*legendre[atLm(l,0)]
                    ...
                    for(int m = 1 ; m <= l ; ++m){
                        {
                            const FReal coef = FMath::pow(FReal(-1.0),l) * FMath::pow(r,l) * legendre[atLm(l,m)] / FReal(fact(l+m));
                            ...
                            // F(p) += -2 x m x Imag(LI)
                            Fp += 2 * FReal(m) * (u[atLm(l,m)].getReal() * I_real + u[atLm(l,m)].getImag() * I_imag);
                        }
                        {
                            const FReal legendre_l_minus_1 = (m == l) ? 0 : FReal(l+m)*legendre[atLm(l-1,m)];
                            const FReal coef = (FMath::pow(FReal(-1.0),l)/FReal(fact(l+m))) * FMath::pow(r,l) * ((FReal(l) * sph.getCosTheta()*legendre[atLm(l,m)]
            ...
            { // Result for potential
                ...
                for(int l = 0 ; l <= P ; ++l ){
                    {
                        // (l-|m|)! * P{l,0} / r^(l+1)
                        //const FReal coef = FMath::pow(r,l) * fact(l+0) * legendre[atLm(l,0)];// 2.0 Australia paper
                        //const FReal coef = fact(l-0) * legendre[atLm(l,0)] / FMath::pow(r,l+1);// 2.0 Other paper
                        const FReal coef = FMath::pow(FReal(-1.0),l) * FMath::pow(r,l) * legendre[atLm(l,0)] / FReal(fact(l+0));
                        const FReal I_real = coef * FMath::Cos(FReal(0) * sph.getPhi() + i_pow_m[0]);
                        const FReal I_imag = coef * FMath::Sin(FReal(0) * sph.getPhi() + i_pow_m[0]);
                        magnitude += u[atLm(l,0)].getReal() * I_real - u[atLm(l,0)].getImag() * I_imag;
                        //printf("harmo[%d][%d] = %f \n", l,0,u[atLm(l,0)].getReal() * I_real - u[atLm(l,0)].getImag() * I_imag);
                    }
                    for(int m = 1 ; m <= l ; ++m ){
                        //const FReal coef = FMath::pow(r,l) * fact(l+m) * legendre[atLm(l,m)];// 2.0 Australia paper
                        //const FReal coef = fact(l-m) * legendre[atLm(l,m)] / FMath::pow(r,l+1);// 2.0 Other paper
                        const FReal coef = FMath::pow(FReal(-1.0),l) * FMath::pow(r,l) * legendre[atLm(l,m)] / FReal(fact(l+m));
                        const FReal I_real = coef * FMath::Cos(FReal(m) * sph.getPhi() + i_pow_m[m & 0x3]);
                        const FReal I_imag = coef * FMath::Sin(FReal(m) * sph.getPhi() + i_pow_m[m & 0x3]);
                        magnitude += FReal(2.0) * ( u[atLm(l,m)].getReal() * I_real - u[atLm(l,m)].getImag() * I_imag );
                        //printf("harmo[%d][%d] = %f \n", l,m,u[atLm(l,m)].getReal() * I_real - u[atLm(l,m)].getImag() * I_imag);
                    }
                }
                printf("%d>> Potential %f\n",particle.getIndex(), magnitude); // delete me
                // inc potential
    ...
    /** P2P
      * This function procceed the P2P by particlesMutualInteraction
      */
