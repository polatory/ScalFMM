 ...
 * @class FChebTensorialM2LHandler
 ...
 * r\f$.
 *
 * PB: IMPORTANT! Remark on Tensorial version
 * Due to the UCB structure (more precisely the fact that U and B are applied 
 * at M2M/L2L steps) we had rather not compress each component of the tensorial 
 * kernel. We cannot either perform a low rank approx of the 
 * dim*ninteractions*nnodes*nnodes matrix because then the application of the tensor would be tricky. 
 * The only way to make it work efficiently is to shift the application of U 
 * and B to the M2L step if the kernel is tensorial... Not an easy task!
 * However the symmetric version of Chebyshev should extend easily to 
 * tensorial kernels.
 *
 * @tparam ORDER interpolation order \f$\ell\f$
 */
template <int ORDER, class MatrixKernelClass>
class FChebTensorialM2LHandler : FNoCopyable
{
	enum {order = ORDER,
  ...
  FReal *B[dim];
	...
	unsigned int rank[dim];   //<! truncation rank, satisfies @p epsilon
...
public:
	FChebTensorialM2LHandler(const FReal _epsilon)
  : MatrixKernel(),/* U(NULL), C(NULL), B(NULL),*/ epsilon(_epsilon)/*, rank(0)*/
	...
	{
    for (unsigned int d=0; d<dim; ++d){
      if (U[d] != NULL) delete [] U[d];
      if (B[d] != NULL) delete [] B[d];
    ...
    unsigned long sizeM2L = 0;

    for (unsigned int d=0; d<dim; ++d){

      // check if aready set
      if (U[d]||C[d]||B[d]) throw std::runtime_error("Compressed M2L operator already set");

      rank[d] = ComputeAndCompress(epsilon, U[d], C[d], B[d], d);

      sizeM2L += 343*rank[d]*rank[d]*sizeof(FReal);
	...
	{
		FChebTensorialM2LHandler<ORDER,MatrixKernelClass>::ComputeAndCompressAndStoreInBinaryFile(epsilon);
	 ...
	 */
	static unsigned int ComputeAndCompress(const FReal epsilon, FReal* &U, FReal* &C, FReal* &B, const int d);
	 ...
	 */
	unsigned int getRank(const unsigned int d) const {return rank[d];}
  ...
  /**
	 ...
	 */
  void applyU(const FReal *const X, FReal *const x, const unsigned int d) const
  {
    FBlas::gemva(nnodes, rank[d], 1., U[d], const_cast<FReal*>(X), x);
  }
              ...
              const unsigned int idxK) const
  {
    ...
    FBlas::gemva(rank[d], rank[d], scale, C[d] + idx*rank[d]*rank[d] + d*rank[d]*rank[d]*343, const_cast<FReal*>(Y), X);
  }
  ...
  {
		const FReal scale(MatrixKernel.getScaleFactor(CellWidth));
    FBlas::gemva(rank[d], rank[d] * 343, scale, C[d], const_cast<FReal*>(Y), X);
  }
  ...
  /**
	 ...
	 */
  void applyB(FReal *const y, FReal *const Y, const unsigned int d) const
  {
    FBlas::gemtv(nnodes, rank[d], 1., B[d], y, Y);
  }
...
template <int ORDER, class MatrixKernelClass>
unsigned int
FChebTensorialM2LHandler<ORDER, MatrixKernelClass>::ComputeAndCompress(const FReal epsilon,
																															FReal* &U,
																															FReal* &C,
																															FReal* &B,
                                                              const int d)
{
  std::cout<< "Compute and Compress" <<std::endl;
...
//template <int ORDER, class MatrixKernelClass>
//void
//FChebTensorialM2LHandler<ORDER, MatrixKernelClass>::ComputeAndCompressAndStoreInBinaryFile(const FReal epsilon)
//{
//	// measure time
//	FTic time; time.tic();
...
//template <int ORDER, class MatrixKernelClass>
//void
//FChebTensorialM2LHandler<ORDER, MatrixKernelClass>::ReadFromBinaryFileAndSet()
//{
//	// measure time
//	FTic time; time.tic();
