...
public:
	...
	if(nbProcs == 1){
		...
		if(pack.idProc != myRank && 0 < (pack.elementTo-pack.elementFrom)){
		    ...
		    // Send the data
		    for(FSize idxMess = 0 ; idxMess < nbPartsPerPackToSend[idxPack]; idxMess += MAX_BYTE_PER_MPI_MESS){
			const int nbElementsInMessage = int(FMath::Min(nbPartsPerPackToSend[idxPack]-idxMess, MAX_BYTE_PER_MPI_MESS));
		...
		for(unsigned int idxPack = 0; idxPack < packsToRecv.size(); ++idxPack){
		    const FEqualize::Package& pack = packsToRecv[idxPack];
		    if(pack.idProc != myRank && 0 < (pack.elementTo-pack.elementFrom)){
			for(FSize idxMess = 0 ; idxMess < nbPartsPerPackToRecv[idxPack]; idxMess += MAX_BYTE_PER_MPI_MESS){
			    const int nbElementsInMessage = int(FMath::Min(nbPartsPerPackToRecv[idxPack]-idxMess, MAX_BYTE_PER_MPI_MESS));
			    requestsParts.emplace_back();
