...
void   adaptativeTreeBuilSminC(OctreeClass & tree,const int sminM, const int sminL) {
	...
	do{  // leaves level = NbLevels - 1
		...
		octreeIterator.getCurrentCell()->setCellAdaptative();
		if(nbPart <= sminM){
...
//!  \fn   adaptativeTreeBuildLists(OctreeClass & tree) {
...
//!
template  <class OctreeClass >
void   adaptativeTreeBuildLists(OctreeClass & tree) {
	//
	typename OctreeClass::Iterator octreeIterator(&tree) ;
	//
	int NbLevels = tree.getHeight();
	...
	for(int idxLevel = NbLevels ; idxLevel > 1 ; --idxLevel){
		...
		do {
			...
			//								    																														  << "  isAdapt ? " << std::boolalpha<<myCell.isAdaptative()<<  std::endl ;
			//
			...
			while(findParent.moveUp() ) {
				...
				//											    																										   << "    " <<findParent.level()  <<   "   "<< std::boolalpha<< findParent.getCurrentCell()->isAdaptative()  <<  std::endl ;
				if ( findParent.getCurrentCell()->isAdaptative() ){
					//
					//				std::cout << "                      I found " <<std::endl;
					//				myFA.level = findParent.level() ;
					//				myFA.cell   = findParent.getCurrentCell() ;
					myCell.addAdaptativeParent(findParent.getCurrentCell(), findParent.level()) ;
					//						std::cout <<  "     FA      "  << *(myFA.cell) <<std::endl;

					if(myCell.isAdaptative()){
						//		myFA.cell->addAdaptativeChild(&myCell,idxLevel) ;
						findParent.getCurrentCell()->addAdaptativeChild(&myCell,idxLevel) ;
...
void  TulipExport( std::ofstream& tlp, long int nbCell, OctreeClass & tree) {
	...
	//
	// Set adaptative property
	//
	std::string rootColor("  \"(255,0,0,255)\" ") ;      // Bleu
	std::string adaptativeColor("  \"(0,0,255,255)\" ") ;      // Bleu
	...
	for(int idxLevel = 1 ; idxLevel < NbLevels-1 ;  ++idxLevel){
		do{
			if(octreeIterator.getCurrentCell()->isAdaptative()){
				tlp << "  (node " <<  octreeIterator.getCurrentCell()->getGlobalId()   <<adaptativeColor<<"  )" << std::endl ;;
			}
		} while(octreeIterator.moveRight());
		octreeIterator.moveDown() ;
		octreeIterator.gotoLeft();
